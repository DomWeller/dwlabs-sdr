...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === "/") {
      ...
      const models = await pool.query("SELECT metric_name,dimensions ->> 'provider' provider,dimensions ->> 'model' model,count(*) calls,round(avg(metric_value),0) latency_ms,count(*) FILTER (WHERE dimensions ->> 'outcome'='error') errors FROM ops.metrics_events WHERE metric_name IN ('agent_turn','model_call') AND created_at>NOW()-INTERVAL '7 days' GROUP BY metric_name,dimensions ->> 'provider',dimensions ->> 'model' ORDER BY calls DESC LIMIT 20");
      const modelRows = models.rows.map((row) => `<tr><td>${escapeHtml(row.metric_name)}</td><td>${escapeHtml(row.provider)}</td><td>${escapeHtml(row.model)}</td><td>${escapeHtml(row.calls)}</td><td>${escapeHtml(row.latency_ms)}</td><td>${escapeHtml(row.errors)}</td></tr>`).join("");
      return send(res, 200, page("Visão geral", `<h1>Visão geral</h1><div class="grid">${cards}</div><div class="card"><h2>Leads por origem</h2><table><tr><th>Origem</th><th>Leads</th><th>Fechados</th></tr>${originRows}</table></div><div class="card"><h2>Ferramentas — últimos 7 dias</h2><table><tr><th>Ferramenta</th><th>Chamadas</th><th>Latência média (ms)</th><th>Erros</th></tr>${toolRows}</table></div><div class="card"><h2>Agente/modelo — últimos 7 dias</h2><table><tr><th>Métrica</th><th>Provedor</th><th>Modelo</th><th>Chamadas</th><th>Latência média (ms)</th><th>Erros</th></tr>${modelRows}</table><p class="muted">agent_turn mede a latência percebida do turno completo; model_call aparece quando o runtime fornece o evento de modelo. Custo estimado permanece indisponível até o provedor entregar tokens e tabela de preço confiáveis; o painel não inventa esse valor.</p></div>`));
    }
